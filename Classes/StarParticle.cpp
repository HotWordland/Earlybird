...
void showStarParticleEffect(int color,Point position,Node* node){
    ParticleExplosion* effect = ParticleExplosion::create();            effect->setTexture(Director::getInstance()->getTextureCache()->addImage("star.png"));
/*
    switch (color) {
        case 0:
        {
            effect->setTexture(Director::getInstance()->getTextureCache()->addImage("starLu.png"));

        }
            break;
            case 1:
        {
            effect->setTexture(Director::getInstance()->getTextureCache()->addImage("starLove.png"));

        }
            break;
            case 2:
        {
            effect->setTexture(Director::getInstance()->getTextureCache()->addImage("starYou.png"));
